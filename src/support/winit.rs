        ...
        if let WinitEvent::WindowEvent { event, .. } = self {
            ...
            match event {
                WindowEvent::KeyboardInput { event, .. } => {
                    ...
                    match event.state {
                        ElementState::Pressed => keyboard.press(&event.logical_key),
                        ElementState::Released => keyboard.release(&event.logical_key),
                    };
                }
...
mod tests {
    ...
    fn create_devices() {
        ...
        keyboard.press(&Key::Character("H".into()));
        keyboard.set_modifiers(&Modifiers::default());
    ...
    // fn make_keyboard_event(pressed: bool, key: Key) -> WinitEvent<()> {
    //     use winit::event::*;
    //     use winit::keyboard::*;
    ...
    //     let event = make_keyboard_event(true, key.clone());
    //     let mut keyboard = keyboard();
